 ...
 *
 *@author José Miguel Aguado Coca y Alejandro Martín Casas
 *@date Diciembre 2024
 *
 *
 */
...
private:
	...
	/**
	  * @brief Metodo añadido por nosotros que creará el primer tipo de árbol.
	  * El árbol menos eficiente.
	  */
	bintree<Pregunta> crear_arbol_malo(vector<string> atributos,
									int indice_atributo,
									vector<string> personajes,
	...
	/**
	  * @brief Metodo añadido por nosotros que creará el segundo tipo de árbol.
      * El árbol medio eficiente.
      */
	void crear_arbol_medio(bintree<Pregunta> &arbol_malo, bintree<Pregunta>::node nodoRaiz);

	/**
      * @brief Metodo añadido por nosotros que creará el tercer tipo de árbol.
      * El árbol más eficiente.
      */
	bintree<Pregunta> crear_arbol_bueno(vector<string> atributos,
									vector<string> personajes,
									vector<bool> personajes_restantes,
									vector<vector<bool>> tablero);
	...
	/**
	  ...
	  * Llamará a las 3 funciones distintas de árboles. Se comentan aquellas que no se usen.
	  * Se divide el código para modularizar.
	  */
...
public:
	...
	*/
	...
	/**
	  * @brief Método auxiliar para crear el vector de profundidades de nuestro árbol
	  * @param jugada_actual Nodo del que partimos para medir profundidades, será el nodo raíz
	  *
	  * @return Devuelve el vector de profundidades de las hojas del árbol
	  *
	  * @pre El arbol de preguntas debe haber sido construido previamente
	  */
	vector<int> profundidades_hojas(const bintree<Pregunta>::node& jugada_actual) const;
	/**
	...
	// METODO ADICIONAL 1, DOXYGEN!!
	/**
     * @brief Método que devuelve una cadena con las preguntas formuladas hasta el momento
     * @param jugada Nodo que recorre el árbol
     * @return String con las preguntas formuladas hasta el momento
     */
	string preguntas_formuladas (bintree<Pregunta>::node jugada);


	// METODO ADICIONAL 2, DOXYGEN!!
	/**
     * @brief Método que añade un personaje al tablero
     * @param nombre Nombre del personaje a insertar
     * @param caracteristicas Vector de booleanos con las características del personaje
     * @param nombre_imagen_personaje Nombre de la imagen del personaje
     * @pre El personaje a insertar no puede tener las mismas características que otro personaje
     */
	void aniade_personaje (string nombre, vector <bool> caracteristicas, string nombre_imagen_personaje="");


	// METODO ADICIONAL 3, DOXYGEN!!
	/**
     * @brief Método que elimina un personaje del tablero
     * @param nombre Nombre del personaje a eliminar
     * @pre El personaje debe existir en el tablero
     */
